...
export default function FlightsPage() {
  ...
  const [columnVisibilityModel, setColumnVisibilityModel] = useState(
    JSON.parse(localStorage.getItem(LOCAL_STORAGE_HIDDEN_COLUMNS) || "{}") || {}
  );
  ...
  const showResetColumnsSelection =
    typeof columnVisibilityModel === "object" &&
    Object.keys(columnVisibilityModel).length === 0;

  return (
    <div style={{ margin: "auto", position: "relative", width: "80%" }}>
      <h1 onClick={() => setShowHeavy(true)}> Flights </h1>
      {!showResetColumnsSelection ? (
        <Button onClick={resetColumnsSelection}>
          {" "}
      ...
      ) : null}
      {
        <Button
          onClick={() => {
            setPaginationLimit(paginationLimit + INCREASE_LIMIT);
          }}
        >
          {" "}
          Load{" "}
